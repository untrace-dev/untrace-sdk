...
// Evidently AI provider configuration schema
const EvidentlyConfigSchema = z.object({
  apiKey: z.string().min(1, 'API key is required'),
  enabled: z.boolean().default(true),
  endpoint: z.string().optional(),
  options: z
    .object({
      baseUrl: z.string().url().default('https://api.evidentlyai.com'),
...
export class EvidentlyProvider extends BaseIntegrationProvider {
  readonly name = 'Evidently AI';
  readonly destinationId = 'evidently';
  ...
  constructor(config: ProviderConfig, destinationId: string) {
    ...
    this.baseUrl =
      (config.options?.baseUrl as string) || 'https://api.evidentlyai.com';
    ...
    if (!this.workspace) {
      throw new Error('Evidently AI workspace is required');
    }
  ...
  async validateConfig(
    ...
    try {
      EvidentlyConfigSchema.parse(config);
      return { success: true };
    } catch (error) {
      return {
  ...
  private addTraceToBuffer(trace: TraceData, context: TraceContext): void {
    ...
    if (this.dataBuffer.length >= this.batchSize) {
      this.flushToEvidently();
    }
  }

  private async flushToEvidently(): Promise<void> {
    ...
    try {
      await this.sendBatchToEvidently(dataToFlush);
    } catch (error) {
      console.error(
        '[EvidentlyProvider] Failed to flush data to Evidently AI:',
        error,
      );
  ...
  private async sendBatchToEvidently(
    data: Array<{
      trace: TraceData;
      context: TraceContext;
      timestamp: string;
    ...
    // Transform data to Evidently format
    const evidentlyTraces = data.map((item) => ({
      duration: item.trace.duration || 0,
      evaluation: {
        drift_score: item.trace.attributes?.['evidently.drift_score'] || 0,
        hallucination_detected:
          item.trace.attributes?.['evidently.hallucination_detected'] || false,
        performance_score:
          item.trace.attributes?.['evidently.performance_score'] || 0,
        quality_score: item.trace.attributes?.['evidently.quality_score'] || 0,
        toxicity_score:
          item.trace.attributes?.['evidently.toxicity_score'] || 0,
      },
      llm: {
    ...
    for (let attempt = 0; attempt <= this.retryAttempts; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Evidently AI API error: ${response.status} ${response.statusText} - ${errorText}`,
          );
        }
  ...
  async destroy(): Promise<void> {
    ...
    await this.flushToEvidently();
  }
}

// Evidently AI provider factory
export class EvidentlyProviderFactory implements ProviderFactory {
  createProvider(
    config: ProviderConfig,
    destinationId: string,
  ): EvidentlyProvider {
    return new EvidentlyProvider(config, destinationId);
  }
...
// Evidently AI test runner
export class EvidentlyTestRunner extends BaseDestinationTestRunner {
  async testConnection(config: Record<string, unknown>): Promise<TestResult> {
    try {
      const apiKey = config.apiKey as string;
      const baseUrl =
        (config.baseUrl as string) || 'https://api.evidentlyai.com';
      ...
      if (!apiKey) {
        return {
          ...
          message: 'API key is required for Evidently AI destinations',
          success: false,
        };
      }
      ...
      if (!workspace) {
        return {
          ...
          message: 'Workspace is required for Evidently AI destinations',
          success: false,
        };
      }

      // Test connection by making a request to Evidently AI's API
      ...
      if (!response.ok) {
        return {
          ...
          message: `Evidently AI connection test failed: ${response.status} ${response.statusText}`,
          success: false,
        };
      }
      ...
      return {
        ...
        message: `Evidently AI connection test to ${baseUrl} successful`,
        success: true,
      };
    } catch (error) {
      return {
        ...
        message: `Evidently AI connection test failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
        success: false,
      };
    }
  }
    ...
    try {
      ...
      if (!apiKey || !workspace) {
        return {
          ...
          message:
            'API key and workspace are required for Evidently AI destinations',
          success: false,
        };
      }
      ...
      if (!response.ok) {
        return {
          ...
          error: `HTTP ${response.status}: ${response.statusText}`,
          message: `Evidently AI delivery test failed: ${response.status} ${response.statusText}`,
          success: false,
        };
      }
      ...
      return {
        ...
        message: `Evidently AI delivery test to ${baseUrl} successful`,
        success: true,
      };
    } catch (error) {
      return {
        ...
        error: error instanceof Error ? error.message : 'Unknown error',
        message: `Evidently AI delivery test failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
        success: false,
      };
    }
  }
  ...
  async validateConfig(config: Record<string, unknown>): Promise<TestResult> {
    try {
      ...
      return {
        ...
        message: 'Evidently AI destination configuration is valid',
        success: true,
      };
    } catch (error) {
      return {
...
// Register the Evidently AI provider and test runner
registerProvider(new EvidentlyProviderFactory());
registerDestinationTestRunner('evidently', EvidentlyTestRunner);
