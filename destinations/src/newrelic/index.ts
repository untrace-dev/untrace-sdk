...
// New Relic provider configuration schema
const NewRelicConfigSchema = z.object({
  ...
  options: z
    .object({
      ...
      baseUrl: z.string().url().default('https://api.newrelic.com'),
...
export class NewRelicProvider extends BaseIntegrationProvider {
  readonly name = 'New Relic';
  readonly destinationId = 'newrelic';
  ...
  constructor(config: ProviderConfig, destinationId: string) {
    ...
    this.baseUrl =
      (config.options?.baseUrl as string) || 'https://api.newrelic.com';
    this.accountId = (config.options?.accountId as string) || '';
  ...
  async validateConfig(
    ...
    try {
      NewRelicConfigSchema.parse(config);
      return { success: true };
    } catch (error) {
      return {
  ...
  private addTraceToBuffer(trace: TraceData, context: TraceContext): void {
    ...
    if (this.dataBuffer.length >= this.batchSize) {
      this.flushToNewRelic();
    }
  }

  private async flushToNewRelic(): Promise<void> {
    ...
    try {
      await this.sendBatchToNewRelic(dataToFlush);
    } catch (error) {
      console.error(
        '[NewRelicProvider] Failed to flush data to New Relic:',
        error,
      );
  ...
  private async sendBatchToNewRelic(
    data: Array<{
      trace: TraceData;
      context: TraceContext;
      timestamp: string;
    ...
    // Transform data to New Relic format
    const newRelicEvents = data.map((item) => ({
      duration: item.trace.duration || 0,
      eventType: 'LlmCompletion',
      llm: {
        conversation_id:
          item.trace.attributes?.['llm.conversation_id'] || item.trace.id,
        id: item.trace.id,
        metadata: {
    ...
    const endpoint = this.buildEndpoint('/graphql');
    await this.makeRequestWithRetry(endpoint, {
      body: JSON.stringify({
        query: `
					mutation($accountId: Int!, $events: [LlmCompletionEventInput!]!) {
						llmCompletionEvents(accountId: $accountId, events: $events) {
							success
							errors {
								message
							}
    ...
    for (let attempt = 0; attempt <= this.retryAttempts; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `New Relic API error: ${response.status} ${response.statusText} - ${errorText}`,
          );
        }
  ...
  async destroy(): Promise<void> {
    ...
    await this.flushToNewRelic();
  }
}

// New Relic provider factory
export class NewRelicProviderFactory implements ProviderFactory {
  createProvider(
    config: ProviderConfig,
    destinationId: string,
  ): NewRelicProvider {
    return new NewRelicProvider(config, destinationId);
  }
...
// New Relic test runner
export class NewRelicTestRunner extends BaseDestinationTestRunner {
  async testConnection(config: Record<string, unknown>): Promise<TestResult> {
    try {
      const apiKey = config.apiKey as string;
      const baseUrl = (config.baseUrl as string) || 'https://api.newrelic.com';
      ...
      if (!apiKey) {
        return {
          ...
          message: 'API key is required for New Relic destinations',
          success: false,
        };
      }
      ...
      if (!accountId) {
        return {
          ...
          message: 'Account ID is required for New Relic destinations',
          success: false,
        };
      }

      // Test connection by making a request to New Relic's API
      const testEndpoint = `${baseUrl}/graphql`;
      ...
      if (!response.ok) {
        return {
          ...
          message: `New Relic connection test failed: ${response.status} ${response.statusText}`,
          success: false,
        };
      }
      ...
      return {
        ...
        message: `New Relic connection test to ${baseUrl} successful`,
        success: true,
      };
    } catch (error) {
      return {
        ...
        message: `New Relic connection test failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
        success: false,
      };
    }
  }
    ...
    try {
      ...
      if (!apiKey || !accountId) {
        return {
          ...
          message:
            'API key and account ID are required for New Relic destinations',
          success: false,
        };
      }
            ...
            events: [
              {
                ...
                eventType: 'LlmCompletion',
                llm: {
                  conversation_id: trace.id || 'test-trace-delivery',
      ...
      if (!response.ok) {
        return {
          ...
          error: `HTTP ${response.status}: ${response.statusText}`,
          message: `New Relic delivery test failed: ${response.status} ${response.statusText}`,
          success: false,
        };
      }
      ...
      return {
        ...
        message: `New Relic delivery test to ${baseUrl} successful`,
        success: true,
      };
    } catch (error) {
      return {
        ...
        error: error instanceof Error ? error.message : 'Unknown error',
        message: `New Relic delivery test failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
        success: false,
      };
    }
  }
  ...
  async validateConfig(config: Record<string, unknown>): Promise<TestResult> {
    try {
      ...
      return {
        ...
        message: 'New Relic destination configuration is valid',
        success: true,
      };
    } catch (error) {
      return {
...
// Register the New Relic provider and test runner
registerProvider(new NewRelicProviderFactory());
registerDestinationTestRunner('newrelic', NewRelicTestRunner);
