...
// Middleware provider configuration schema
const MiddlewareConfigSchema = z.object({
  ...
  options: z
    .object({
      baseUrl: z.string().url().default('https://api.middleware.io'),
      ...
      serviceName: z.string().default('untrace-service'),
      timeout: z.number().min(5000).max(60000).default(30000), // 30 seconds
    })
    .optional(),
});

export class MiddlewareProvider extends BaseIntegrationProvider {
  readonly name = 'Middleware';
  readonly destinationId = 'middleware';
  ...
  constructor(config: ProviderConfig, destinationId: string) {
    ...
    this.baseUrl =
      (config.options?.baseUrl as string) || 'https://api.middleware.io';
    this.serviceName =
      (config.options?.serviceName as string) || 'untrace-service';
  ...
  private addTraceToBuffer(trace: TraceData, context: TraceContext): void {
    ...
    if (this.dataBuffer.length >= this.batchSize) {
      this.flushToMiddleware();
    }
  }

  private async flushToMiddleware(): Promise<void> {
    ...
    try {
      await this.sendBatchToMiddleware(dataToFlush);
    } catch (error) {
      console.error(
        '[MiddlewareProvider] Failed to flush data to Middleware:',
        error,
      );
  ...
  private async sendBatchToMiddleware(
    data: Array<{
      trace: TraceData;
      context: TraceContext;
      timestamp: string;
    ...
    // Transform data to OpenTelemetry format for Middleware
    const otelSpans = data.map((item) => ({
      resourceSpans: [
        {
          resource: {
            attributes: [
              ...
              spans: [
                {
                  attributes: [
                    ...
                    // Add other attributes from item.trace.attributes
                    ...Object.entries(item.trace.attributes || {}).map(
                      ([key, value]) => ({
                        key,
                        value: {
                  ...
                  parentSpanId: '', // No parent span for top-level trace
                  spanId: item.trace.id, // Using trace ID as span ID for simplicity
                  startTimeUnixNano:
                    new Date(item.trace.timestamp).getTime() * 1000000,
                  status: {
                    code: item.trace.status === 'error' ? 2 : 1, // ERROR = 2, OK = 1
                  },
    ...
    for (let attempt = 0; attempt <= this.retryAttempts; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Middleware API error: ${response.status} ${response.statusText} - ${errorText}`,
          );
        }
  ...
  async destroy(): Promise<void> {
    ...
    await this.flushToMiddleware();
  }
}

// Middleware provider factory
export class MiddlewareProviderFactory implements ProviderFactory {
  createProvider(
    config: ProviderConfig,
    destinationId: string,
  ): MiddlewareProvider {
    return new MiddlewareProvider(config, destinationId);
  }
...
export class MiddlewareTestRunner extends BaseDestinationTestRunner {
  async testConnection(config: Record<string, unknown>): Promise<TestResult> {
    try {
      const apiKey = config.apiKey as string;
      const baseUrl = (config.baseUrl as string) || 'https://api.middleware.io';
      ...
      if (!apiKey) {
        return {
          ...
          message: 'API key is required for Middleware destinations',
          success: false,
        };
      }

      // Test connection by making a request to Middleware's API
      ...
      if (!response.ok) {
        return {
          ...
          message: `Middleware connection test failed: ${response.status} ${response.statusText}`,
          success: false,
        };
      }
      ...
      return {
        ...
        message: `Middleware connection test to ${baseUrl} successful`,
        success: true,
      };
    } catch (error) {
      return {
        ...
        message: `Middleware connection test failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
        success: false,
      };
    }
  }
  ...
  async testDelivery(
    ...
    try {
      const apiKey = config.apiKey as string;
      const baseUrl = (config.baseUrl as string) || 'https://api.middleware.io';
      const serviceName = (config.serviceName as string) || 'untrace-service';
      ...
      // Test delivery by sending a sample OpenTelemetry trace
      const testEndpoint = `${baseUrl}/v1/traces`;
      const response = await fetch(testEndpoint, {
        body: JSON.stringify([
          {
      ...
      if (!response.ok) {
        return {
          ...
          error: `HTTP ${response.status}: ${response.statusText}`,
          message: `Middleware delivery test failed: ${response.status} ${response.statusText}`,
          success: false,
        };
      }
      ...
      return {
        ...
        message: `Middleware delivery test to ${baseUrl} successful`,
        success: true,
      };
    } catch (error) {
      return {
        ...
        error: error instanceof Error ? error.message : 'Unknown error',
        message: `Middleware delivery test failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
        success: false,
      };
    }
  }
  ...
  async validateConfig(config: Record<string, unknown>): Promise<TestResult> {
    try {
      ...
      return {
        ...
        message: 'Middleware destination configuration is valid',
        success: true,
      };
    } catch (error) {
      return {
...
// Register the Middleware provider and test runner
registerProvider(new MiddlewareProviderFactory());
registerDestinationTestRunner('middleware', MiddlewareTestRunner);
